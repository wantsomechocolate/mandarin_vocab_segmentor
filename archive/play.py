...
## Sample Input
#text = "我是一个红色的苹果"
with open('sampleinput.txt', encoding = 'utf8') as fh:
    text = fh.read()
...
## I'm interested in knowing if a single character was only included as part of a larger word or not
## In order to accomplish this, I need to know if a longer character was found directly after the smaller character. 
st = dict()
big_word_list =[]
...
while left < len(text):
    ...
    right = left+1
    #print(text[left:right], left, right)

    #if text[left:right] in stopwords:
    #    pass
    
    while text[left:right] in dct and right < len(text) + 1:
        entered_while = True
        #print(text[left:right], left, right)

        #if text[left:right] in st:
        #    st[text[left:right]]['counter']+=1
        #else:
        #    st[text[left:right]] = dict(counter = 1) 
        #st.add(text[left:right])
        
        right = right+1

    # you just exited the while, so add the last thing you found - this won't work if you don't enter the while loop at least once though. 
    if entered_while == True:
        if text[left:right-1] in st:
            st[text[left:right-1]]['counter']+=1
        else:
            st[text[left:right-1]] = dict(counter = 1) 
    left = left+1
...
for key,val in st.items():
	print(key,":",val['counter'],":",dct[key])

##Brute Force
#for i in range(len(samplesentence)):
#    for j in range(i,len(samplesentence)+1):
#        if samplesentence[i:j] in dct:
#            st.add(samplesentence[i:j])
#            print(samplesentence[i:j],dct[samplesentence[i:j]])
